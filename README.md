# M-038 — Skin Lesion Segmentation

ISIC 2018 dermoscopy skin-lesion segmentation.
...
> This is a dermoscopy image. Segment the skin lesion (melanoma or non-melanoma) using a green mask, and overlay a semi-transparent green fill plus contour on every frame that contains the lesion.

## S3 Raw Data

```
s3://med-vr-datasets/M-038_ISIC2018/raw/
```
...
```
data/questions/skin_lesion_segmentation_task/
├── task_0000/
│   ├── first_frame.png
│   ├── final_frame.png
│   ├── first_video.mp4
...
|---|---|---|
| `domain` | `"skin_lesion_segmentation"` | Task domain string used in output paths. |
| `s3_bucket` | `"med-vr-datasets"` | S3 bucket containing raw data. |
| `s3_prefix` | `"M-038_ISIC2018/raw/"` | S3 key prefix for raw data. |
...
```
M-038_isic2018_skin_lesion_seg_data-pipeline/
